...
def test_uci():
    ...
    #data=["mushroom","adult","ionosphere"]
...
def cascade(X,y):
    ...
    while run:
        ...
        run, max_acc=calculate_score(y_test,final_prediction,prev_ac,prev_p,prev_r,acc,pre,rec,max_acc)
        prev_ac=acc
        prev_p=pre
        prev_r=rec
        ...
        if not run:
            print("memory requirement:", memory)
...
def calculate_score(y_test,final_prediction,prev_ac,prev_p,prev_r,acc,pre,rec,max_acc):
